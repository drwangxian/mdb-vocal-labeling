...
class TONet(nn.Module):
    ...
    def __init__(self):
        ...
        self.l_model = FTAnet(freq_bin=360, time_segment=128)
        # r_model for Tone-CFP
        self.r_model = FTAnet(freq_bin=360, time_segment=128)
        self.mode = 'all'
        self.centf = np.array(get_CenFreq(32, 2050, 60))
        self.centf[0] = 0
        self.loss_func = None
        self.max_metric = np.zeros((3, 6))
        if self.mode == "all" or self.mode == "tcfp":
            assert self.r_model is not None, "Enabling TONet needs two-branch models!"
    ...
    def forward(self, x, tx = None):
        if self.mode == "single":
            ...
            _, output_l = self.l_model(x)  # [6, 1, 361, 128], feature
            ...
            feature_agg_mi = self.tcfp_linear(feature_agg) # [6, 360, 128], logits
            bm_agg = torch.cat((bm_l, bm_r), dim = 2)  # [6, 1, 2, 128]
            bm_agg = bm_agg.squeeze(dim = 1)  # [6, 2, 128]
            bm_agg_mi = self.tcfp_bm(bm_agg)  # [6, 1, 128], logits
            ...
            octave_prob = self.octave_decoder(octave_feature)  # [6, 6, 128], no real probs, instead it is logits

            tone_bm = self.tone_bm(bm_agg)  # [6, 128, 2] --> [6, 128, 1], logits
            octave_bm = self.octave_bm(bm_agg)  # [6, 128, 2] --> [6, 128, 1], logits
            tone_bm = tone_bm.permute(0,2,1)  # [6, 1, 128]
            octave_bm = octave_bm.permute(0,2,1)  # [6, 1, 128]

            tone_prob = torch.cat((tone_bm, tone_prob), dim=1)  # [6, 13, 128]
            octave_prob = torch.cat((octave_bm, octave_prob), dim=1)  # [6, 7, 128]
            
            final_feature = torch.cat((tone_prob, octave_prob, feature_agg_mi, bm_agg_mi), dim=1)  # [6, 381, 128]
            ...
            return dict(chroma=tone_prob, octave=octave_prob, pitch=final_feature)
    ...
    def training_step(self, batch, batch_idx):
        ...
        gds = torch.tensor(gds, device=device_type)  # (6, 128)
